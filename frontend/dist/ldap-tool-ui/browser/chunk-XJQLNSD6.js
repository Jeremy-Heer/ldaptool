...
// src/app/services/ldap.service.ts
var LdapService = class _LdapService {
  http;
  API_BASE = "/ldap";
  constructor(http) {
    this.http = http;
  ...
  testConnection(connection) {
    ...
    return this.http.post(`${this.API_BASE}/test-connection`, request, { headers }).pipe(catchError((error) => {
      const errorResult = {
        success: false,
  ...
  search(connection, searchRequest) {
    ...
    const headers = this.buildHeaders(connection);
    return this.http.post(`${this.API_BASE}/ldapsearch`, request, { headers }).pipe(catchError((error) => {
      const errorResult = {
        success: false,
        entries: [],
  ...
  getRootDse(connection) {
    ...
    const headers = this.buildHeaders(connection);
    return this.http.post(`${this.API_BASE}/get-root-dse`, request, { headers }).pipe(map((response) => {
  ...
  getRootDn(connection) {
    ...
    const headers = this.buildHeaders(connection);
    return this.http.post(`${this.API_BASE}/get-root-dn`, request, { headers }).pipe(map((response) => {
   ...
   */
  loadTreeLevel(connection, baseDn) {
    const searchRequest = {
      base: baseDn,
      filter: "(objectClass=*)",
      scope: "one",
    ...
    return this.search(connection, searchRequest).pipe(map((result) => {
   ...
   * Load entry details
   */
  loadEntryDetails(connection, dn) {
    const searchRequest = {
      base: dn,
      filter: "(objectClass=*)",
      scope: "base",
  ...
  modify(connection, modifyRequest) {
    ...
    const headers = this.buildHeaders(connection);
    return this.http.post(`${this.API_BASE}/ldapmodify`, request, { headers }).pipe(catchError((error) => {
      const errorResult = {
        success: false,
   ...
   */
  browseChildren(connection, parentDn) {
    const searchRequest = {
      base: parentDn,
      filter: "(objectClass=*)",
      scope: "one",
  ...
  hasChildrenIndicators(entry) {
    ...
    return objectClasses.some((oc) => organizationalClasses.includes(oc.toLowerCase()));
  }
  /**
  ...
  getNodeIcon(node) {
    if (node.isRootDse)
      return "fa-server";
    if (node.isNamingContext)
      return "fa-sitemap";
    if (!node.entry?.attributes?.["objectClass"])
      return "fa-folder";
    const objectClasses = Array.isArray(node.entry.attributes["objectClass"]) ? node.entry.attributes["objectClass"] : [node.entry.attributes["objectClass"]];
  ...
  getEntryIcon(node) {
    ...
    if (objectClasses.includes("domain"))
      return "domain";
    if (objectClasses.includes("organization"))
      return "business";
    return "folder_open";
  }
  ...
  static \u0275fac = function LdapService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _LdapService)(\u0275\u0275inject(HttpClient));
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _LdapService, factory: _LdapService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(LdapService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var LocalStorageService = class _LocalStorageService {
  CONNECTIONS_KEY = "ldapConnections";
  LAST_CONNECTION_KEY = "ldapLastConnection";
  MAX_SAVED_CONNECTIONS = 200;
  ...
  saveConnection(connection) {
    ...
    const connectionWithTimestamp = __spreadProps(__spreadValues({}, connection), {
   ...
   */
  generateBookmarkUrl(connection) {
...
@angular/forms/fesm2022/forms.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-XJQLNSD6.js.map
