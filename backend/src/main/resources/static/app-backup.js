...
.controller('LdapController', ['$scope', '$http', function($scope, $http) {
    ...
    $scope.saveConnection = function() {
        if (!$scope.newConnectionName.trim()) {
            alert('Please enter a name for this connection');
            return;
        }
        
        var connection = {
            name: $scope.newConnectionName.trim(),
    ...
    $scope.autoConnectToBrowse = function() {
        ...
        // First, get the Root DN from the server
        $scope.getRootDn()
            .then(function(rootDn) {
                $scope.browseRequest.base = rootDn;
                // Load the root level with the detected DN
                return $scope.loadTreeLevel(rootDn, null);
            })
    ...
    // Pre-populate some example values for development/testing
    $scope.connectionConfig.host = 'localhost';
    $scope.connectionConfig.port = 389;
    $scope.searchRequest.base = 'dc=example,dc=com';
    $scope.searchRequest.filter = '(objectClass=*)';
    $scope.attributesString = 'cn,mail,telephoneNumber';
    
    $scope.browseRequest.base = 'dc=example,dc=com';
    
    // Ensure default tab is active
    $scope.setActiveTab('connection');
    
    // Initialize connection settings from URL or localStorage
    // Initialize connection when controller loads
